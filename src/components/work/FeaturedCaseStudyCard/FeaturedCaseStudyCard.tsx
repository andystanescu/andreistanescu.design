...
export function FeaturedCaseStudyCard({ slug, title, description, thumbnail, passwordRequired, category = "CASE STUDY", year = "" }: { slug: string; title: string; description: string; thumbnail: string; passwordRequired: boolean; category?: string; year?: string }) {
  ...
  return <button type="button" className={styles.featured} onClick={() => passwordRequired ? setUnlocking(true) : window.location.href = `/work/${slug}`}><div className={styles.featuredVisual} style={thumbnail ? { backgroundImage: `url(${thumbnail})` } : undefined} aria-hidden="true"><span className={styles.featuredMeta}><i aria-hidden="true" />{category}{year ? ` · ${year}` : ""}</span></div><div className={styles.featuredCopy}><p className="label-eyebrow" style={{ color: "var(--text-on-deep-accent)" }}>FEATURED CASE STUDY</p><h2 className="heading-01">{title}</h2><p className="body-default" style={{ color: "var(--text-on-deep-secondary)" }}>{description}</p><span className={styles.featuredLink}>Read case study <ArrowIcon size={16} /></span></div></button>;
}
