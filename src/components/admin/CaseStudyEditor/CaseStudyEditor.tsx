  ...
  return (
    <form ref={formRef} data-editor-page className={`${adminStyles.form} ${styles.editorForm}`} action={action ?? `/api/admin/case-studies/${study.id}`} method="POST" encType="multipart/form-data" onInput={scheduleDraftSave} onChange={scheduleDraftSave}>
      <header className={styles.editorHeader}>
          ...
          <div className={styles.editorActions}>{saveState !== "idle" && <span className={styles.saveStatus}>{saveState === "saving" ? "Saving…" : saveState === "error" ? "Save failed" : "Saved"}</span>}<a href="/admin/case-studies" className={adminStyles.tertiaryButton}>Cancel</a><button type="submit" name="intent" value={editing ? "publish" : "draft"} className={adminStyles.submit}>{editing ? "Publish" : "Create case study"}</button></div>
      ...
      <section id="case-study-panel-assessment" role="tabpanel" aria-labelledby="case-study-tab-assessment" hidden={tab !== "assessment"} className={styles.panel} aria-label="ConScept engagement assessment">
        ...
        <div className={styles.engagement}><div className={styles.engagementHeader}><div><h2 className="heading-03">Engagement activities</h2><p className="body-small">Activities are grouped by their primary driving metric. Mark completed work as conducted, or exclude an activity when it did not apply.</p></div></div>{recommendations.length === 0 && <p className="body-small">Add one or more scores to generate recommendations.</p>}{recommendationGroups.map(({ criterion, activities }) => <section className={styles.activityGroup} key={criterion.key}><div className={styles.activityGroupHeader}><h3>{criterion.label}</h3><span>{scores[criterion.key] || "—"} / 5</span></div><div className={styles.activityGrid}>{activities.map((activity) => { const excluded = notApplicable.includes(activity.name); return <article key={activity.name} className={`${styles.activityCard} ${excluded ? styles.activityCardExcluded : ""}`}><div className={styles.activityInfo}><strong>{activity.name}</strong><p className={styles.activityDescription}>{getActivityDescription(activity.name)}</p>{activity.triggeredBy.length > 1 && <small>Also driven by {activity.triggeredBy.slice(1).map((trigger) => assessmentCriteriaList.find((item) => item.key === trigger.criterion)?.label).filter(Boolean).join(" · ")}</small>}</div><div className={styles.activityControls}>{!excluded && <><input type="hidden" name="assessment_likely" value={activity.name} /><label className={styles.activityChoice}><input type="checkbox" name="assessment_conducted" value={activity.name} defaultChecked={assessment.conducted.includes(activity.name)} /><span>Conducted</span></label></>}<label className={styles.activityChoice}><input type="checkbox" name="assessment_not_applicable" value={activity.name} checked={excluded} onChange={(event) => setNotApplicable((current) => event.target.checked ? [...current, activity.name] : current.filter((name) => name !== activity.name))} /><span>Not applicable</span></label></div></article>; })}</div></section>)}</div>
      </section>
