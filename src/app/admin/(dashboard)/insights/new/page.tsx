...
export default async function NewInsightPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return <>{error && <p style={{ color: "var(--border-error)" }}>{error}</p>}<InsightEditor action="/api/admin/insights" categories={categories} settingsAuthor={settings.author_name} relatedReadings={getRelatedReadingOptions()} authorAvatarUrl={settings.about_hero_image} /></>;
}
